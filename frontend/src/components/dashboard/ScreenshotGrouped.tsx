  ...
  return (
    <div className="space-y-8">
      {groups.map((group) => {
        ...
        const localHour = convertUTCHourToLocal(group.hour);
        ...
        return (
          <div key={group.hour} className="space-y-4">
            <h3 className="text-xl font-semibold text-gray-900 flex items-center">
              ...
              <span className="ml-3 text-gray-600">Hour {localHour}</span>
            </h3>

            {group.intervals.map((interval) => {
              // ✅ Convert UTC times to local times
              const localStartTime = convertUTCTimeToLocal(interval.startTime);
              const localEndTime = convertUTCTimeToLocal(interval.endTime);
